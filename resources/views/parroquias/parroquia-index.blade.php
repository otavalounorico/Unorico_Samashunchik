<x-app-layout>
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        ...
        <div class="px-5 py-4 container-fluid">
            <div class="alert alert-dark text-sm"><strong style="font-size:24px;">Gestión de Parroquias</strong></div>
            ...
            <div class="d-flex gap-2 mb-3">
                <a href="{{ route('parroquias.create') }}" class="btn btn-success">
                    <i class="fa-solid fa-plus"></i> Nueva Parroquia
                </a>
                <form method="GET" class="d-flex gap-2 ms-auto" style="max-width: 640px;">
                    <select name="canton_id" class="form-select" style="max-width: 260px;">
                    ...
                    <a href="{{ route('parroquias.index') }}" class="btn btn-link">Limpiar</a>
                </form>
            </div>
                        ...
                        <table class="table table-hover table-bordered align-middle text-center">
                            ...
                            <tbody>
                                @forelse($parroquias as $p)
                                    <tr>
                                        <td>{{ $p->id }}</td>
                                        <td class="fw-semibold"><a href="{{ route('parroquias.show',$p) }}">{{ $p->nombre }}</a></td>
                                ...
                                @empty
                                    <tr><td colspan="4" class="text-muted py-4">No hay parroquias.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    <div class="mt-3">{{ $parroquias->links() }}</div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('click', function (e) {
            ...
            Swal.fire({
                title: '¿Eliminar parroquia?', html:`¿Eliminar <b>"${item}"</b>?`,
