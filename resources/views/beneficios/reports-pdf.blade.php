...
<head>
    ...
    <style>
        ...
        /* Clases utilitarias para alineación */
...
<body>
    ...
    <main>
        ...
        <div class="reporte-titulo">REPORTE DE BENEFICIOS</div>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        @foreach ($headings as $index => $heading)
                            {{-- Saltamos el ID de la base de datos (index 0) para usar el contador visual # --}}
                            @if ($index > 0)
                                <th 
                                    @if ($heading == 'Código') style="width: 60px;" 
                                    @elseif ($heading == 'Nombre del Beneficio') style="text-align: left;"
                                    @elseif ($heading == 'Tipo') style="width: 50px;"
                                    @elseif ($heading == 'Valor ($)') style="width: 60px; text-align: right;"
                                    @elseif ($heading == 'Fecha Creación') style="width: 70px;"
                                    @endif
                                >
                                    {{ $heading }}
                                </th>
                ...
                <tbody>
                    @foreach ($data as $index => $row)
                        <tr>
                            <td style="font-weight: bold; background-color: #e9ecef;">{{ $loop->iteration }}</td>
                            
                            {{-- Convertimos el objeto row a array y quitamos el primer elemento (ID) --}}
                            @foreach (array_slice((array)$row, 1) as $key => $cell)
                                <td 
                                    {{-- Lógica para alinear texto vs números --}}
                                    @if ($key === 'nombre' || $key === 'descripcion') class="text-left" 
                                    @elseif ($key === 'valor') class="text-right font-bold"
                                    @endif
                                >
                                    {{ $cell }}
                                </td>
